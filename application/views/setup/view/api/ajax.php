...
function RefreshTable() {
  ...
  dataSource = '<?= base_url("$url/list_data/set_$tbl") ?>';
  if (oTable)
      oTable.fnDestroy();
  ...
  if (fnServerObjectToArray) {
    var oTable = $("#fileData").dataTable({
      ...
      "columnDefs": [
        ...
        {
          "searchable": false, "orderable": false, className: "text-center", "targets": 5,
          render: function(data, type, row) {
            idnya = data.id_x; <?php $tblnya="set_$tbl"; ?>
            detail = "aksi('detail','"+idnya+"','','<?= $url_modal; ?>/<?= $tblnya; ?>')";
            ...
            <?php if (check_permission('view', 'update', "setup/api")): ?>
            edit  = "aksi('edit','"+idnya+"','sync_form','<?= $url_modal; ?>/<?= $tblnya; ?>')";
            ...
            <?php endif; ?>
            <?php if (check_permission('view', 'delete', "setup/api")): ?>
            hapus = "aksi('hapus','"+idnya+"','','<?= $url_hapus; ?>/<?= $tblnya; ?>')";
